...
public static class LoadDataArgsExtensions
{
    ...
    public static PagedQuery ToPagedQuery(this LoadDataArgs loadDataArgs)
    {
        return new PagedQuery
        {
            OrderBy = loadDataArgs.GetOrderBy(),
            Page = loadDataArgs.GetPageNumber(),
            PageSize = loadDataArgs.GetPageSize()
        };
    }
}
